...
def remove_build_folder():
    if os.path.exists(Config.BUILD_FOLDER):
        ...
        print(f"{Config.BUILD_FOLDER} does not exist")
...
def get_cmake_command(action):
    ...
    elif action in (Action.BUILD_DEBUG, Action.BUILD_REALEASE):
        configuration = (
            Configuration.Debug
...
def generate_project_files():
    ...
    print(f"Generated profect files with command: {command}")
...
def run_clang_format(source_dir):
    ...
    if not format_sources:
        print(f'No sourse files found in {source_dir}')
        return
    ...
    if result.returncode == 0:
        ...
        print('Error runnung clang-format.')
if __name__ == "__main__":
    ...
    action_map = {
        ...
        Action.BUILD_REALEASE: lambda: build_project(Configuration.Release),
        Action.CLANG_FORMAT: lambda: run_clang_format(Config.SOURSE_DIR)
    }
